...
**`app.py`** — FastAPI application factory (`build_app()`). Mounts all routers. It doesn't wire dependencies itself — `ApiServer` (`server.py`), built by `trading.di.containers.components.build_components()`, resolves the dependencies (session factory, `KiteClient`, `OrderExecutor`, etc.) and passes them in as plain constructor arguments.
