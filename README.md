...
**Each module owns its config.** Every pipeline stage is an `AbstractRegistry` subclass with its own Pydantic config model living alongside it — `TickConfig` + `TickIngestor` in `tick_ingest/service/ingestor.py`, `RiskConfig` + `RiskFilter` in `risk/service/filter.py`, and so on. Reading one module's `service/` file tells you everything about that stage — what it needs, what it produces, and what it persists.
...
**Backtests reuse live code exactly.** `SignalGenerator`, `RiskFilter`, and `OrderExecutor` run unchanged in `trading-integ-tests`' and `trading-research`'s backtesting frameworks — those repos import this repo as a dependency and drive the same classes against a simulated data source and broker instead of Zerodha. If a strategy behaves differently in backtesting than in live trading, it is a data or timing difference, not a code difference.
