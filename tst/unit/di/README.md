...
| File | What it tests |
|------|--------------|
| `test_container.py` | `build_infra()`: `Settings`, `AsyncEngine`, `async_sessionmaker`, `TradingStore`, `AuditStore` construction, and that the `engine=` override (swapping in an in-memory sqlite engine) actually takes effect |
